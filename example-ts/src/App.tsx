...
const App: React.FC = () => {
  ...
  const handleClickAdd = useCallback(() => {
    ...
    setItems([
      ...items, {id: Date.now(), label: inputText, completed: false}
    ]);
    setInputText("");
  ...
  const handleChangeItem = useCallback((todo: Todo) => {
    setItems(items.map(item => item.id === todo.id ? {
      ...item,
      completed: !item.completed
    } : item));
  }, [items]);

  const handleChangeFilter = useCallback((option: StatusType) => {
    setOption(option);
  }, [option]);
  ...
  const filter = useMemo(() => <StatusFilter onChange={handleChangeFilter} />, [handleChangeFilter]);
